...
class GreedyWordSwapWIR(SearchMethod):
    ...
    def _get_index_order(self, initial_text):
        ...
        if self.wir_method == "unk":
            ...
            leave_one_results, search_over = self.get_goal_results(leave_one_texts,enable_interpret =False)
    ...
    def _perform_search(self, initial_result):
        ...
        while i < len(index_order) and not search_over:
            ...
            if len(transformed_text_candidates) == 0:
                continue
            results, search_over = self.get_goal_results(transformed_text_candidates,enable_interpret = True)
            # exit(0)
            # print(results)
            # print(transformed_text_candidates)
            # exit(0)
            results = sorted(results, key=lambda x: -x.score)
            # cur_result.append(results[0])
