...
class GoalFunction(ABC):
    ...
    def init_attack_example(self, attacked_text, ground_truth_output):
        ...
        self.num_queries = 0
        result, _ = self.get_result(attacked_text,enable_interpret=True, setval=True, check_skip=True)
        print("Initial Sentence:",result.attacked_text)
        return result, _
    ...
    def get_result(self, attacked_text, enable_interpret, setval=False, **kwargs):
        """A helper method that queries ``self.get_results`` with a single
        ``AttackedText`` object."""
        results, search_over = self.get_results([attacked_text],enable_interpret,setval, **kwargs)
        result = results[0] if len(results) else None
        return result, search_over

    def get_results(self, attacked_text_list, enable_interpret,setval=False, check_skip=False):
        ...
        budget.
        """
        print("Enabled Interpretation:",enable_interpret)
        print("SetVal:",setval)
        results = []
        ...
        if enable_interpret:
            ...
            for sample,cnt in zip(attacked_text_list,cnts):
                ...
                # print(batch_encoded['input_ids'].size())
                ...
                c_s= com_scores(atts,encoded)
                t_s= topk(atts,encoded)
                com_scores_list.append(c_s.item())
                topk_scores_list.append(t_s.item())
                print("Candidate:",sample.text,com_scores(atts,encoded).item(),topk(atts,encoded).item(),torch.argmax(model_outputs[cnt]).item())
            
            if setval:
                self.maxcom = com_scores(atts,encoded).item()
                # self.maxk = topk(atts,encoded).item()

        

        indexlist = range(len(attacked_text_list))
        for attacked_text, raw_output, idx in zip(attacked_text_list, model_outputs, indexlist):
            ...
            if enable_interpret:
                goal_function_score = abs(com_scores_list[idx] - self.maxcom)
            else:
                goal_function_score = self._get_score(raw_output, attacked_text)
